...
sys_env_set_status(envid_t envid, int status)
{
	...
	if (status != ENV_RUNNABLE && status != ENV_NOT_RUNNABLE){
		cprintf("set_status: param error\n");
		return -E_INVAL;
	}
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	// check virtual address & permission
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if((uint32_t)srcva >= UTOP || PGOFF(srcva) != 0 || (uint32_t)dstva >= UTOP || PGOFF(dstva) != 0){
		return -E_INVAL;
	}
	...
	if((page = page_lookup(srce->env_pgdir, srcva, &ppte)) == NULL || ((*ppte) & PTE_P) == 0){
		return -E_INVAL;
	}
	
	if((*ppte | PTE_W) == 0 && (perm | PTE_W) != 0){
		return -E_INVAL;
	}
